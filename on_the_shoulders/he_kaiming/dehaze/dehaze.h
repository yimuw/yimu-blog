...
public:
    ...
    cv::Mat recover_J(const cv::Mat &haze_im_in, const cv::Mat &t_in, const cv::Vec3f &atmospheric_light)
    {
        ...
        for(int row_idx = 0; row_idx < t.rows; ++row_idx)
        {
            for(int col_idx = 0; col_idx < t.cols; ++col_idx)
            {
                ...
                J.at<cv::Vec3f>(row_idx, col_idx) =                
                    (haze_bgr - atmospheric_light) / robust_t + atmospheric_light;
            }
        }
    ...
    cv::Mat find_dark_channel(const cv::Mat &m, const int window_half_size)
    {
        ...
        for(int row_idx = window_half_size; row_idx < m_pad.rows - window_half_size; ++row_idx)
        {
            ...
            {
                dark_chaneel.at<float>(row_idx - window_half_size, col_idx - window_half_size) = find_min_in_rbg_patch(m_pad, row_idx, col_idx, window_half_size);
            }
        }
    ...
    cv::Vec3f compute_atmospheric_light(const cv::Mat &dark_channel, const cv::Mat &haze_im)
    {
        ...
        std::vector<index_and_intensity> candidate_from_dark(
            dark_channel_index_and_intensities.end() - dark_channel_index_and_intensities.size() * 0.001,
            dark_channel_index_and_intensities.end());    

        std::cout << "candidate_from_dark.size():" << candidate_from_dark.size() << std::endl;

        // show_pixel(haze_im, candidate_from_dark, "candidate_from_dark");

        std::vector<index_and_intensity> index_and_im_intensity;
        for(const auto &p : candidate_from_dark)
        {
            const cv::Vec3f bgr= haze_im.at<cv::Vec3f>(p.first);
            const float B = bgr[0];
            const float G = bgr[1];
            const float R = bgr[2];
            const float intensity = ( (0.3 * R) + (0.59 * G) + (0.11 * B) );
            index_and_im_intensity.emplace_back(p.first, intensity);
        }

        std::sort(index_and_im_intensity.begin(), index_and_im_intensity.end(),
        [](const index_and_intensity &l, const index_and_intensity &r){
            return l.second < r.second;
        });

        // show_pixel(haze_im, {index_and_im_intensity.back()}, "highest intensity");

        cv::Point2d atmospheric_light_idx = index_and_im_intensity.back().first;
        return haze_im.at<cv::Vec3f>(atmospheric_light_idx);
    }
